...
function DrSignUp(){
  ...
  const [specialization , setSpcialization] = useState()
  ...
  const [specializationError , setSpcializationError] = useState(false)
  ...
  const onSpecializationChange = (input) => {
    setSpcialization(input.target.value)
    setSpcializationError(false)
  }
...
<div className='row container'>
    ...
    <div className="mb-3">
      ...
      <div className="mb-3">
      {specializationError && <div className='alert alert-danger'>Please select one specialization</div>}
        <label htmlFor="exampleInputPassword1" className="form-label">Specialization</label>
        <select className="form-select" aria-label="Default select example" onChange={onSpecializationChange}>
  <option >Please Select One</option>
  <option value="Emergency Medicine">Emergency medicine</option>
  <option value="Allergy and immunology">Allergy and immunology</option>
  <option value="Diagonostic radiology">Diagnostic radiology</option>
  ...
  <option value="Chest Specialist">Chest Specialist</option>
      ...
      <button onClick={signup} style={{float:"right"}} type="submit" className="btn btn-primary">Sign up</button>
